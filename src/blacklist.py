...
class Blacklists(Resource):
	...
	def get(self):
		return jsonify(
			Result = {
				"Status":"Success",
				"Message":"Showing All Blacklisted IPs"
			},
			IPAddresses=BLModel.get_all_ip()
		)

class BlacklistsCreate(Resource):
	@jwt_required
	def post(self):
		...
		#insert new IP into database as blacklisted value
		new_ip = BLModel(ipv4 = data['IPv4'], ipv6 = data['IPv6'], start_date = data['Start_Date'],
